...
class APB_driver extends uvm_driver #(APB_xtn);
  ...
  task drive();
    ...
    while (Pvif.APB_DRV_CB.Pselx == 0) @(Pvif.APB_DRV_CB);

    if (Pvif.APB_DRV_CB.Pwrite == 0) Pvif.APB_DRV_CB.Prdata <= $random;  //temp

    else Pvif.APB_DRV_CB.Prdata <= 'bx;

    repeat (2) @(Pvif.APB_DRV_CB);
